   ...
   initV1: function (baseline) {
    ...
    extractCSV: function(lines)  {
        ...
        for (var i = 0; i < lines.length; i++) { 
                if(lines[i].includes(".csv")) {
                   while(!lines[i].startsWith("Content-Transfer-Encoding")) {
                            i++;
                   }
                   
                   if (lines[i].includes("base64"))  {
                       console.log("base64");
                        while(!lines[i].startsWith("X-Attachment-Id")) {
                            i++;
                        }
                        ...
                        console.log("text");
                        while(!lines[i].startsWith("start, end")) {
                            i++;
                        }
                   }
